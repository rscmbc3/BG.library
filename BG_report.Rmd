...
##BG value stats by hour for ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
BGvalue_Summary
```
##Sensor value stats by hour for ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
Sensorvalue_Summary
```

##BG high>150 count for past ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
BGHigh_Count
```

##BG very high count for past ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
BGveryHigh_Count
```

##BG low count for past ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
BGLow_Count
```

##BG good value count (>80 and <150) psat ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
BGgood_Count
```
...
##Suspend basal on low count past ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
suspendBasal_Count
```


##BG value by time and date with mean values past ```{r, eval=TRUE}numberDays``` days
...
##Sensor value by time and date with mean values past ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE,fig.width=14, fig.height=10}
...
##carb intake max values by time and date paste ```{r, eval=TRUE}numberDays``` days
```{r, eval=TRUE}
carbs_timeDaytable
...
plotLine_ly(allData,  scatterOnly = FALSE, pointSize = 10,
            ...
            addBolusType = "",   barSubPlot = FALSE,     
            plotSummary = "Sensor.Glucose..mg.dL.",
            addSetting =c("basal","carbRatio","corrFactor"),filterCond = "",
            legendInset = -0.2)
```
